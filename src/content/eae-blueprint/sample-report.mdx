...
<Accordion>
  ...
  <AccordionItem title="1. System Overview">
    EAE 리포트 엔진은 컴포넌트 기반 아키텍처를 따릅니다.
    
    MDX 파일을 작성하면, 이것이 React 컴포넌트로 변환되고, Tailwind CSS로 스타일이 적용되어 
    완성된 웹 애플리케이션으로 배포됩니다.
    
    모든 리포트는 동일한 Universal Structure를 따르며, 이를 통해 일관성 있는 학습 경험을 제공합니다.
  </AccordionItem>

  <AccordionItem title="2. Universal Structure (보편적 구조)">
    ```
    리포트 구조:
    ┌─────────────────────────────────────┐
  ...
  <AccordionItem title="4. Data Flow / Workflow">
